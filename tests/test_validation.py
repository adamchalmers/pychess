...
def test_validation_core():
  ...
  with BadMove: g.move(Move(0,7,0,6, WHITE, g))
  # Can't move an empty square
  with BadMove: g.move(Move(4,4,0,6, WHITE, g))
  # Can't move an opponent's piece
  with BadMove: g.move(Move(0,1,0,2, WHITE, g))
  # Can't move if it's not your turn
  with BadMove: g.move(Move(0,1,0,2, BLACK, g))
  # Can't move invalid coordinates
  with BadMove: g.move(Move(0,8,0,5, WHITE, g))
  with BadMove: g.move(Move(0,-1,0,5, WHITE, g))
  with BadMove: g.move(Move(8,6,0,5, WHITE, g))
  with BadMove: g.move(Move(-1,6,0,5, WHITE, g))
  with BadMove: g.move(Move(0,6,-8,5, WHITE, g))
  with BadMove: g.move(Move(0,6,17,5, WHITE, g))
  with BadMove: g.move(Move(0,6,0,9, WHITE, g))
  with BadMove: g.move(Move(0,6,0,-2, WHITE, g))
  # Can't move a non-player
  with BadMove: g.move(Move(0,1,0,2, 3003, g))
...
def test_validation_pawn():
  ...
  wp.x, wp.y = 4,4
  bp.x, bp.y = 4,3
  with BadMove: g.move(Move(4,4,4,3, WHITE, g))
  with BadMove: g.move(Move(4,4,3,3, WHITE, g))
  with BadMove: g.move(Move(4,4,5,3, WHITE, g))
  g.turn = BLACK
  with BadMove: g.move(Move(4,3,4,3, BLACK, g))
  with BadMove: g.move(Move(4,3,4,4, BLACK, g))
  with BadMove: g.move(Move(4,3,3,4, BLACK, g))
  with BadMove: g.move(Move(4,3,5,4, BLACK, g))
  ...
  with BadMove: g.move(Move(1,3,1,5, BLACK, g))
  g.move(Move(1,3,1,4, BLACK, g))
  with BadMove: g.move(Move(6,4,6,2, WHITE, g))
...
def test_validation_bishop():
  ...
  with BadMove: g.move(Move(5, 7, 4, 6, WHITE, g))
  with BadMove: g.move(Move(5, 7, 4, 8, WHITE, g))
  with BadMove: g.move(Move(5, 7, 6, 6, WHITE, g))
  with BadMove: g.move(Move(5, 7, 6, 8, WHITE, g))
  ...
  # Can't move to 6,0
  g.turn = WHITE
  with BadMove: g.move(Move(2,4,6,0, WHITE, g))
  # Can move to 5,1
...
def test_validation_rook():
  ...
  assert wr.char == "R"
  with BadMove: g.move(Move(7,7,7,6, WHITE, g))
  with BadMove: g.move(Move(7,7,7,5, WHITE, g))
  wr.x = 4
  wr.y = 4
  g.move(Move(4,4,0,4, WHITE, g))
  g.turn = WHITE
  g.move(Move(0,4,7,4, WHITE, g))
  g.turn = WHITE
  g.move(Move(7,4,7,2, WHITE, g))
  g.turn = WHITE
  g.move(Move(7,2,7,5, WHITE, g))
  g.turn = WHITE
  g.move(Move(7,5,7,2, WHITE, g))
  g.turn = WHITE
  with BadMove: g.move(Move(7,2,6,1, WHITE, g))
